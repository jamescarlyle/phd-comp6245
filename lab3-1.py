...
def train_logistic_regression(X, y_true, alpha, iterations):
   ...
   # Iterate
    for i in range(iterations):
        gradient = calculate_gradient(X_bias, y_true, weights)
        gradients.append(np.linalg.norm(gradient))
        losses.append(nll_loss(y_true, sigmoid(X_bias @ weights)))
...
# weights_nls, gradients_nls, losses_nls = train_logistic_regression(X_train_nls, y_train_nls, 0.1, 1000)

# plt.title("Gradient and Loss for non-Linearly-Seperable by Iteration")
...
# weights_nls, gradients_nls, losses_nls = train_logistic_regression(X_train_ls, y_train_ls, 0.1, 5000)

# plt.title("Gradient and Loss for Linearly-Seperable by Iteration")
# plt.xlabel("Iteration")
# plt.ylabel("Gradient / Loss")
# plt.plot(gradients_nls, label="Gradient")
# plt.plot(losses_nls, label="Loss")
...
def nll_loss_with_penalty(y_true, y_pred, weights, lmbda):
    ...
    # J(w) = NLL(w) + λ||w||^2
    penalty = lmbda * np.sum(weights**2)
    return loss + penalty
...
def train_logistic_regression_with_penalty(X, y_true, alpha, iterations, lmbda):
   ...
   # Iterate
    for i in range(iterations):
        gradient = calculate_gradient_with_penalty(X_bias, y_true, weights, lmbda)
        gradients.append(np.linalg.norm(gradient))
        losses.append(nll_loss_with_penalty(y_true, sigmoid(X_bias @ weights), weights, lmbda))
        weights -= alpha * gradient
    return weights, gradients, losses

weights_nls, gradients_nls, losses_nls = train_logistic_regression_with_penalty(X_train_ls, y_train_ls, 0.1, 5000, 0.1)
